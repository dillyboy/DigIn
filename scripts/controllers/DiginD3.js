...
!function (exports){
  ...
  DiginD3.parser = function (delimiter) {
  ...
  // Tree
  DiginD3.models.set('tree', function(){

    var tree = DiginD3.model();
  ...
  // Points
  DiginD3.models.set('points', function(){

    var points = DiginD3.model();
  ...
  // Graph
  DiginD3.models.set('graph', function(){

    var graph = DiginD3.model();
  ...
  // Identity
  DiginD3.models.set('identity', function(){
    return DiginD3.model();
  })

  DiginD3.models.tree = DiginD3.models.get('tree');
  DiginD3.models.points = DiginD3.models.get('points');
  DiginD3.models.graph = DiginD3.models.get('graph');
  DiginD3.models.identity = DiginD3.models.get('identity');
  ...
  DiginD3.chart = function (id) { 

    var id = id || DiginD3.charts.values().length,
        title = "Untitled",
        description = null,
        category = null,
        thumbnail = "",
    ...
    var chart = function (selection) {
      selection.each(function (data){
        dDiginD3.call(this, selection, model(data));
      })
    }
    ...
    function dDiginD3(selection, data) {
      return;
    }
    ...
    chart.dDiginD3 = function(_) {
      if (!arguments.length) return dDiginD3;
      dDiginD3 = _;
      return chart;
    }
  ...
  DiginD3.isString = function(value){
    return typeof value == 'string';
  }

  DiginD3.isBoolean = function(value){
  ...
  DiginD3.isNumber = function(value) {
    return !isNaN(value);
  }

  DiginD3.isDate = function(value){ 
    var isDate = false;
    for (var format in DiginD3.dateFormats){
      if (value.match(DiginD3.dateFormats[format])) {
  ...
  DiginD3.typeOf = function (value) {
    if (value === null || value.length === 0) return null;
    if (DiginD3.isDate(value)) return Date.name;
    if (DiginD3.isNumber(value)) return Number.name;
    if (DiginD3.isString(value)) return String.name;
    return null;
  }

  DiginD3.divergingRange = function(n){
    return d3.range(n).map(function (d){
      return d3.hsl( 360 / n * d, .4, .58 ).toString();
    })
  }

  DiginD3.foreground = function(color){
