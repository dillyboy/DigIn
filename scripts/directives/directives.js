...
angular.module('DiginD3.directives', [])
	      ...
	      link: function postLink(scope, element, attrs) {
	        ...
	        scope.scales = [ 
	        	
	        	{
	        		...
	        		value : d3.scale.ordinal().range(DiginD3.divergingRange(1)),
	        		reset : function(domain){ this.value.range(DiginD3.divergingRange(domain.length || 1)); },
	        ...
	        function listColors(){
	        	scope.colors = scope.colorScale.value.domain().map(function (d){
	        		...
	        		if (DiginD3.isNumber(a.key) && DiginD3.isNumber(b.key)) return a.key - b.key;
...
.directive('DiginD3Table', function () {
  return {
    restrict: 'A',
    link: function postLink(scope, element, attrs) {
    	...
    	function sort(a,b) {
    		if (DiginD3.isNumber(a[sortBy]) && DiginD3.isNumber(b[sortBy])) return descending ? a[sortBy] - b[sortBy] : b[sortBy] - a[sortBy];
        ...
        function downloadPng(){
          ...
          var context = canvas.getContext("2d");
					...
					image.onload = function() {
					  context.dDiginD3Image(image, 0, 0);
